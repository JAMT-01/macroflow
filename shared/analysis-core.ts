/**
 * Analysis logic with no database, filesystem, or runtime dependency.
 *
 * Both the Cloudflare Worker (the deployed app) and the local Node benchmark
 * tooling import this module, so the prompt, the Argentine meal-type rules, and
 * the uncertainty maths cannot drift apart between the two.
 */
...
export type OpenRouterMealTypeSuggestion = {
  type?: MealType;
  confidence?: number;
  explanation?: string;
};
...
export function normalizeScaleReference(reference: ScaleReference | undefined, defaultDiameterCm: number): ScaleReference {
...
export function calculate(food: FoodRow, grams: number, confidence = 0.86, evidence?: { visualEvidence?: string; portionBasis?: string; gramsLow?: number; gramsHigh?: number; uncertaintyReasons?: string[] }): AnalysisItem {
...
export function fallbackMeasurement(reference: ScaleReference, capture?: CaptureMetadata): MeasurementAssessment {
  return {
...
export function matchFood(foods: FoodRow[], name: string): FoodRow | undefined {
...
export function analyzeDescription(foods: FoodRow[], description: string, options: { scaleReference: ScaleReference; localTime: string }): MealAnalysis {
...
export function normalizeMeasurement(parsed: OpenRouterMeasurement | undefined, reference: ScaleReference, capture?: CaptureMetadata): MeasurementAssessment {
...
export function normalizeMealTypeSuggestion(parsed: OpenRouterMealTypeSuggestion | undefined, description: string, localTime: string): MealTypeSuggestion {
...
export function normalizeItem(foods: FoodRow[], item: OpenRouterItem, measurement: MeasurementAssessment, allowTightRange: boolean): AnalysisItem {
...
/** The JSON schema OpenRouter must answer with for a one-photo meal analysis. */
...
/** The JSON schema OpenRouter must answer with when refining an estimate from chat. */
